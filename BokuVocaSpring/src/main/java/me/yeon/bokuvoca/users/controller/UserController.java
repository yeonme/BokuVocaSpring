...
public class UserController {
	...
	final String uploadPath = "/usr/share/nginx/html/tomcat8/upload";
	...
	public String goLogin(@CookieValue(value="Remember",required = false)Cookie rCookie, @CookieValue(value="RPass",required = false)Cookie rPass, Model model, HttpSession session){
		...
		try{
		if(rCookie != null){
			model.addAttribute("username",URLDecoder.decode(rCookie.getValue(),"UTF-8"));
		}
		if(rPass != null){
			model.addAttribute("password",URLDecoder.decode(rPass.getValue(),"UTF-8"));
		}
		}catch(Exception ex){
			logger.error("유저 이름 자동 완성 실패.");
	...
	public String login(JUser user, boolean remember, boolean rpassword, HttpSession session, Model model, HttpServletResponse response){
		...
		if(u != null){
			...
			Cookie cookie2 = new Cookie("RPass",URLEncoder.encode(user.getPassword(),"UTF-8"));
			//리멤버 키로 아이디 저장
			cookie.setPath("/");
			cookie2.setPath("/");
			...
			response.addCookie(cookie2);
				...
				logger.error("Cache 기록 실패. "+user.getUsername()+" / "+user.getPassword());
				logger.error(ex.toString());
			}
			return "redirect:home";
		}
